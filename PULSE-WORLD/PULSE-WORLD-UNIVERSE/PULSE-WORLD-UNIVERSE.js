...
// ============================================================================
//
// We compute a "spin weight" per band, then normalize to a spinVector.
//
// Bands:
//   - firebase
//   - netlify
...
//   - other
//
// This lets you see which band is "dominant" at the universe level.
//
...
function computeBandWeight({ density, pressure, civIndex, costIndex }) {
  ...
  // density + pressure + civilization, with cost as friction
...
export function createPulseWorldUniverse({
  ...
  // ---------------------------------------------------------------------------
  //
  // You can dynamically register/unregister worlds:
  //   - "firebase-main"
  //   - "netlify-main"
  //   - "groundstation-us-west-2"
  //   - "satellite-skyview-1"
  //
  // Each provider is an object with optional snapshot getters.
  // Civilization hints let you inject symbolic context (tier, cost, orbital hints).
  //
  ...
  //
  // This is where we:
  //   - Call provider snapshot functions (if present).
  //   - Derive density/pressure from mesh metrics.
  //   - Apply orbital hints for groundstation/satellite kinds.
  //
  // IMPORTANT:
  //   - If a provider throws, we swallow and treat as null (safeCall).
  //   - If a world has no mesh metrics, density/pressure default low.
  //   - Orbital hints are purely symbolic; you compute them outside.
  //
  ...
  function readWorldSnapshot(worldId, provider) {
    ...
    if (provider.kind === "groundstation" || provider.kind === "satellite") {
      // Orbital bias:
      //   - If sky is active (high contactDensity/orbitalReach),
      //     we gently increase density/pressure.
      //
      //   - This lets the universe "feel" that the sky is alive,
      //     even if userCount/castleCount are low for that world.
      //
      const orbitalBias =
  ...
  //
  // This gives you:
  //   - rawBands: unnormalized weights per band.
  //   - spinVector: normalized weights (sum = 1).
  //   - dominantBand: which band is strongest.
  //
  // You can use this to:
  //   - Decide which world/band to favor for expansion.
  //   - Drive UI (e.g., show "orbital-season" when satellite dominates).
  //
  ...
  //
  // This gives you:
  //   - globalTier/globalTierIndex: highest civilization tier across worlds.
  //   - leadingWorldId: which world holds that tier.
  //   - avgDensity/avgPressure: average across all worlds.
  //   - perWorld: per-world civ + orbital info.
  //
  // You can use this to:
  //   - Decide epoch transitions.
  //   - See which world is "capital" or "legendary".
  //   - Inspect orbital influence per world.
  //
  ...
  // ---------------------------------------------------------------------------
  //
  // This is a coarse universe health signal:
  //   - totalUsers / totalCastles
  //   - maxMeshPressureIndex
  //   - density / pressure
  //   - healthBand: calm / stable / stressed / overloaded
  //
  // You can use this to:
  //   - Gate expansion.
  //   - Trigger "stress" or "collapse" modes.
  //
  ...
  //
  // epochMode:
  //   - prelude / growth / golden-age / stress / collapse
  //
  // season:
  //   - fire-season      (firebase dominant)
  //   - wind-season      (netlify dominant)
  //   - earth-season     (local dominant)
  //   - storm-season     (edge dominant)
  //   - sky-season       (groundstation dominant)
  //   - orbital-season   (satellite dominant)
  //   - void-season      (other / no clear band)
  //
  // You can use this to:
  //   - Drive game narrative.
  //   - Drive UI theming.
  //   - Reflect orbital dominance visually.
  //
  ...
  // ---------------------------------------------------------------------------
  //
  // This is what you serialize / store / inspect:
  //   - identity: minimal universe identity (no external meta).
  //   - worlds: per-world snapshots.
  //   - multispin: band weights + dominant band.
  //   - civilizationField: global + per-world civ.
  //   - healthField: coarse health.
  //   - gameUniverseField: epochMode + season.
  //
  // Your IMMORTAL++ / IndexedDB v30 layer can:
  //   - Take this snapshot.
  //   - Compress it.
  //   - Store it.
  //   - Version it.
