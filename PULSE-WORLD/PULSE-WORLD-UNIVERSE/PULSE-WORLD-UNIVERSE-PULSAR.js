// ============================================
//  PULSE-WORLD-UNIVERSE-PULSAR v2  (PURE ESM)
// ============================================
//
...
//    - Satellite pre-signal hooks
//    - Mesh pressure awareness
//    - Smarter WARM / EARN / SHIFT
//    - Richer events for Universe
//
//  Pure JavaScript.
//  ES Modules only.
// ============================================
...
// --------------------------------------------
// 3. SATELLITE PRE-SIGNAL HOOKS
// --------------------------------------------
//
// Called when we detect the "better signal first":
//   - bumps satelliteWindowScore
//   - marks pre-signal active
//   - emits event for Universe
//
...
function registerSatellitePreSignal(info = {}) {
  ...
  // bump window score optimistically (clamped)
...
// --------------------------------------------
// 6. ROUTING RULES PER PULSE KIND (v2)
// --------------------------------------------
//
// Now aware of:
//  - meshPressure
...
function chooseForSync(w) {
  // If we have a fresh pre-signal and window is decent → satellite
...
function chooseForShift(w) {
  ...
  if (w.satelliteWindowScore > 0.5) return 'MESH_VIA_SATELLITE';
...
function decideRoute(pulse, w = worldState) {
  ...
  switch (pulse.kind) {
    case 'SYNC':
      ...
      reason = 'SYNC: prefer satellite (with pre-signal), else mesh (if healthy), else host.';
      break;
    ...
    case 'SHIFT':
      ...
      reason = 'SHIFT: prefer satellite-assisted redistribution when window is open.';
      break;
