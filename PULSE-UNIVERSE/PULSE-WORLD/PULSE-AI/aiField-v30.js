...
function emitFieldPacket(type, payload, sourceId = "ai-binary-field-v30") {
  return Object.freeze({
    ...
    sourceId,
    timestamp: Date.now(), // IMMORTAL++: wall-clock allowed for packets, not logic
...
class AIBinaryField {
  constructor(config = {}) {
    this.id = config.id || "ai-binary-field-v30";
  ...
  // ---------------------------------------------------------------------------
  //  PACKET GENERATION — binary‑only, packet‑aware v3
  // ---------------------------------------------------------------------------
