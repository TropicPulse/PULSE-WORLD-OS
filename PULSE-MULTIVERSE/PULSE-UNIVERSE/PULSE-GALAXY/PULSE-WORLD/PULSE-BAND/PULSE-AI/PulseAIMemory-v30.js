...
// ============================================================================
//  ORGAN IMPLEMENTATION — v30 IMMORTAL-ADVANTAGE+++ (PulseCoreMemory-only)
// ============================================================================
...
export class AIMemory {
  constructor(config = {}) {
    ...
    // Max bits per value or snapshot
    this.maxBits = config.maxBits || 8192;

    // Reporters (all optional, owner-only surfaces)
  ...
  // --------------------------------------------------------------------------
  //  ARTERY METRICS — v7 (segment + window + shard + hot-key + bias + drift)
  // --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
  ...
  write(keyBin, valueBin) {
    ...
    if (toStore.length > this.maxBits) {
      this._trace("write:truncated", { keyBits: keyBin.length, originalBits: toStore.length });
    ...
    this._trace("write", { keyBits: k.length, valueBits: toStore.length, shardId, artery });
  ...
  delete(keyBin) {
    ...
    const artery = this._computeMemoryArtery();
    this._trace("delete", { keyBits: k.length, shardId, existed, artery });
  ...
  // --------------------------------------------------------------------------
  //  SNAPSHOT — window-safe binary snapshot of memory
  // --------------------------------------------------------------------------
