...
export class AIBinaryReproduction {
  constructor(config = {}) {
    // core wiring
    this.id = config.id || ReproductionMeta.identity;
    this.encoder = config.encoder;          // aiBinaryAgent encoder
    this.genome = config.genome;            // aiBinaryGenome
    this.ancestry = config.ancestry || null;
    this.factory = config.factory;          // organism factory
    ...
    // presence / route / nodeadmin / earn / heartbeat / cortex / memory
    ...
    // extra v30+ surfaces: nervous / evolution / heartbeat-artery / memory-artery
    this.nervousContextProvider =
      typeof config.nervousContextProvider === "function"
        ? config.nervousContextProvider
        : null;

    this.evolutionContextProvider =
      typeof config.evolutionContextProvider === "function"
        ? config.evolutionContextProvider
        : null;

    // NodeAdmin reporter hook (metrics-only)
    ...
    if (!this.encoder) {
      throw new Error("AIBinaryReproduction v30 requires aiBinaryAgent encoder");
    }
    if (!this.genome) {
      throw new Error("AIBinaryReproduction v30 requires aiBinaryGenome");
    }
    if (!this.factory) {
      throw new Error("AIBinaryReproduction v30 requires organism factory");
    }
  }
  ...
  // ---------------------------------------------------------
  //  CONTEXT PROVIDERS (PRESENCE / ROUTE / NODEADMIN / EARN / HEARTBEAT / CORTEX / MEMORY / NERVOUS / EVOLUTION)
// ---------------------------------------------------------
  ...
  _safeNervousContext() {
    if (!this.nervousContextProvider) return null;
    try {
      const ctx = this.nervousContextProvider() || {};
      return {
        routingPressure: this._clamp01(ctx.routingPressure),
        routingBudget: this._clamp01(ctx.routingBudget),
        routesPerSec: typeof ctx.routesPerSec === "number" ? ctx.routesPerSec : 0
      };
    } catch (err) {
      this._trace("nervous:context:error", { error: String(err) });
      return null;
    }
  }

  _safeEvolutionContext() {
    if (!this.evolutionContextProvider) return null;
    try {
      const ctx = this.evolutionContextProvider() || {};
      return {
        driftLevel: typeof ctx.driftLevel === "string" ? ctx.driftLevel : "none",
        newLimb: !!ctx.newLimb,
        overgrowth: !!ctx.overgrowth,
        starvation: !!ctx.starvation
      };
    } catch (err) {
      this._trace("evolution:context:error", { error: String(err) });
      return null;
    }
  }

  // ---------------------------------------------------------
  //  REPRODUCTION ARTERY v7 (EXTENDED METRICS)
// ---------------------------------------------------------
  ...
  _computeReproductionArtery() {
    ...
    const nervousCtx = this._safeNervousContext();
    const evolutionCtx = this._safeEvolutionContext();
    ...
    const artery = {
      ...
      nervous: nervousCtx && {
        routingPressure: nervousCtx.routingPressure,
        routingBudget: nervousCtx.routingBudget,
        routesPerSec: nervousCtx.routesPerSec
      },

      evolution: evolutionCtx && {
        driftLevel: evolutionCtx.driftLevel,
        newLimb: evolutionCtx.newLimb,
        overgrowth: evolutionCtx.overgrowth,
        starvation: evolutionCtx.starvation
      },
  ...
  // ---------------------------------------------------------
  //  COMPRESSED CAPABILITY FINGERPRINT (NODE-ADMIN GRADE)
// ---------------------------------------------------------

  _buildCapabilityFingerprint(genome) {
    // purely symbolic summary of abilities; no secrets, no code
    const caps = genome?.capabilities || {};
    const flags = {
      cortex: !!caps.cortex,
  ...
  _evolveReproductionPacket(packet, extraCtx = {}) {
    ...
    const context = {
      ...
      nervous: this._safeNervousContext(),
      evolution: this._safeEvolutionContext(),
  ...
  _generateReproductionPacket(parentId, childId, genome) {
    const capabilityFingerprint = this._buildCapabilityFingerprint(genome);
    ...
    const payload = {
      ...
      capabilityBits: capabilityFingerprint.bits,
      ownerId: ReproductionMeta.owner.ownerId
    };
    ...
    packet = this._evolveReproductionPacket(packet, {
      ...
      childId,
      genomeFingerprint: genome.fingerprint,
      capabilityFlags: capabilityFingerprint.flags
    });
   ...
   * All abilities preserved; monitored via artery metrics v7.
   */
  cloneOrganism(parentId, parentConfig = {}) {
    const genome = this.genome.loadGenome();
    if (!genome) {
      throw new Error("AIBinaryReproduction v30: no genome available for cloning");
    }
